// Quick parser test: reads a Tide PDF, runs full pipeline, saves Excel output.
// Azure DI results are cached to <pdf>.azure-cache.json to skip re-processing.
// Note: Azure DI sometimes misreads £ as € on Tide PDFs — normalised below.
...
if (!filePath) { console.error('Usage: npx tsx test-tide.ts <path-to-pdf>'); process.exit(1); }
...
if (existsSync(cachePath)) {
    ...
    // Azure DI misreads £ as € on Tide PDFs — normalise before caching
    pageData = pageData.map(p => p ? {
        ...p,
        content: p.content.replace(/€/g, '£'),
        cells: p.cells.map(c => ({ ...c, content: c.content.replace(/€/g, '£') })),
    } : null);
...
const { transactions, statementTotals, ascending } = parseTide(combined);
