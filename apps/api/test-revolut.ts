// Quick parser test: reads a Revolut PDF, runs full pipeline, saves Excel output.
...
if (!filePath) { console.error('Usage: npx tsx test-revolut.ts <path-to-pdf>'); process.exit(1); }
...
// Debug: show raw cells from page 1 (rows 0-5)
if (pageData[0]) {
...
const { transactions } = parseRevolut(combined);
