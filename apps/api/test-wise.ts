/**
 * Wise parser test — processes a real PDF via Azure DI (cached).
 * Results saved as _processed.xlsx next to the PDF.
 *
 * Usage (from apps/api/):
 *   npx tsx test-wise.ts "<path-to-wise-pdf>"
 */
...
import { parse } from './src/services/processing/parsers/wise.js';
...
if (!pdfPath) { console.error('Usage: npx tsx test-wise.ts "<path-to-pdf>"'); process.exit(1); }
...
async function main() {
    ...
    // ── Print table ────────────────────────────────────────────────────────
    const W = { date: 12, type: 10, desc: 46, out: 14, inn: 14 };
    const sep = '─'.repeat(W.date + W.type + W.desc + W.out + W.inn + 14);
    console.log(`\nTransactions: ${txns.length}\n${sep}`);
    console.log(
        'Date'.padEnd(W.date) + 'Type'.padEnd(W.type) +
        'Description'.padEnd(W.desc) + 'Out'.padEnd(W.out) + 'In'.padEnd(W.inn) + 'Balance'
    );
    console.log(sep);
    ...
    console.log(sep);
    console.log('TOTAL'.padEnd(W.date + W.type) + ''.padEnd(W.desc) + totalOut.toFixed(2).padEnd(W.out) + totalIn.toFixed(2));
