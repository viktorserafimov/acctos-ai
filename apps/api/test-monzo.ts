// Quick parser test: reads a Monzo PDF, runs full pipeline, saves Excel output.
...
if (!filePath) { console.error('Usage: npx tsx test-monzo.ts <path-to-pdf>'); process.exit(1); }
...
const { transactions } = parseMonzo(combined);
