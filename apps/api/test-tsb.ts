/**
 * TSB parser end-to-end test.
 *
 * Run from apps/api/:
 *   npx tsx test-tsb.ts "<path-to-pdf>"
 *
...
import { parse } from './src/services/processing/parsers/tsb.js';
import type { Cell } from './src/services/processing/parsers/shared.js';

const pdfPath = process.argv[2];
if (!pdfPath) { console.error('Usage: npx tsx test-tsb.ts "<pdf path>"'); process.exit(1); }

const cachePath = pdfPath.replace(/\.pdf$/i, '') + '_tsb_cache.json';
